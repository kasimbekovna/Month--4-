...
class SocFondUser(User):
    ...
    title = models.CharField(max_length=50, verbose_name='Заявление о назначении пенсии по возрасту', null=True)
    text = models.TextField(verbose_name='Заполните для назначения пенсии основные документы',null=True)
    image = models.ImageField(upload_to='images/', verbose_name='Загрузите паспорт и трудовую книжку', blank=True, null=True)
    pin_number = models.CharField(max_length=16, verbose_name='ИИН -Индивидуальный идентификационный номер', null=True)
    phone_number = models.CharField(max_length=14, default="+996", null=True)
    age = models.PositiveIntegerField(default=58, validators=[MinValueValidator(5), MaxValueValidator(99)], null=True)
    gender = models.CharField(max_length=100, choices=GENDER, null=True)
    stag = models.PositiveIntegerField(default=0, null=True)
    fond = models.CharField(max_length=100, default="Ваш возраст слишком мал для назначении пенсии", null=True)
