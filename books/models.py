...
class Review(models.Model):
    review_book = models.ForeignKey(
        MyBook, on_delete=models.CASCADE, related_name="review_book"
    )
...
class Books(models.Model):
    ...
    audio = models.FileField(
        upload_to="audio/", verbose_name="Загрузите аудио файл", blank=True, null=True
    )
    category_books = models.CharField(
        max_length=50,
        choices=CATEGORY_BOOKS,
        verbose_name="Выберите категорию",
        null=True,
    )

    audio_time = models.PositiveIntegerField(
        verbose_name="Укажите длительность аудио книг", null=True
    )
